...
| `data/sae_jlens_audit/confirmatory_v1_plan_20260711/` | Outcome-blind Llama 3.3 70B SAE/J-lens plan: exact model, public Goodfire SAE, and Neuronpedia lens revisions; 30 static directions; 51 template-family prompts; 1,581 paired prefix-only conditions; frozen lexicons; matched and random controls; source hashes; and no result fields. |
| `data/sae_jlens_audit/confirmatory_v1_20260711/` | Completed Llama 3.3 70B SAE/J-lens forensic release: 420 signed static readouts, 120 sparse-pursuit checkpoints, 1,581 paired prefix-only forwards sharded below GitHub limits, seven-layer trajectories, 20,000-replicate confirmatory and post-run paired-reference analyses, six PNG/PDF figure pairs, exact artifact/runtime records, independent audits, RunPod deletion/cost ledger, and remote/local SHA-256 manifests. |
| `data/sae_jlens_audit/neuronpedia_labels_20260712/` | Compact public provenance snapshot for the pinned Goodfire SAE: 61,850 available Neuronpedia labels, 3,686 missing feature IDs, 484 compressed source-object hashes, source configuration, and snapshot manifest. It strips embeddings and is not an outcome artifact. |
| `data/sae_jlens_audit/confirmatory_v2_calibration_plan_20260712/` | Result-free, independently audited Stage 0 plan for the SAE/J-lens v2 hard-negative follow-up: 138 label-defined candidates, frozen semantic lexicons, exact matching rules, source hashes, and an explicit ban on generations, J-lens outputs, residual states, detector outputs, and target outcomes. It authorizes calibration telemetry only. |
| `data/sae_jlens_audit/confirmatory_v2_calibration_20260712/` | Complete outcome-masked Stage 0 release: 144 target/candidate telemetry rows, all 24 primary-caliper semantic assignments, exact tokenizer lexicons, remote and byte-identical local independent audits, two preserved technical recovery chains, 16-file remote/local checksum verification, RunPod cost/deletion ledger, and release manifest. It contains no behavioral, J-lens, residual, detector, or Stage 1 target outcome. |
| `data/sae_jlens_audit/confirmatory_v2_plan_20260712/` | Final result-free Stage 1 plan: exact 18 A1 and six A2 comparator assignments, 4,029 forward passes, five prompt folds, 14 readers, five stored random projections, residual schema, semantic and replay analysis plans, bound source/calibration hashes, private OSF project record, manifest SHA-256 `47806acf19c5dd56b3ec0c463be5548a08360887b2d777246c1b7f1fbe77893f`, and a passing independent reconstruction. It contains no Stage 1 outcome and does not itself authorize execution; accepted public OSF registration is also required. |
| `data/sae_jlens_audit/confirmatory_v2_20260712/` | Complete public failed-confirmatory release: 4,029 raw readout rows in 16 Git shards; 16 public OSF residual shards; registered replay-gate failure; all-value discrepancy diagnostic; preserved first exploratory serialization failure and byte-identical correction; exploratory A1/A2 and 14-reader analyses; four PNG/PDF figure pairs; independent raw, replay, and endpoint reconstruction; RunPod deletion/cost and OSF publication ledgers; and 82-file release manifest. Confirmatory endpoints are blocked and must not be inferred from the exploratory calculations. |
