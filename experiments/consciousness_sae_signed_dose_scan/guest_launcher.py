#!/usr/bin/env python3
"""Launch only the signed dose-scan runner from provider-attested state."""
...
RUNNER_MODULE = "experiments.consciousness_sae_signed_dose_scan.runner"
...
def _set_attested_environment(
    *, ownership_receipt: Mapping[str, object], environ: Mapping[str, str]
) -> dict[str, str]:
    try:
...
def launch(
    *,
    ...
    argv = (
        ...
        "--ownership-receipt",
        str(receipt_path),
        *_forwarded_arguments(forwarded_args),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"dose-scan guest launch refused: {exc}", file=sys.stderr)
        return 2
    return 0
