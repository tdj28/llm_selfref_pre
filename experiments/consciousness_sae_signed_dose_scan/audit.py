#!/usr/bin/env python3
"""Independently audit and summarize a completed signed dose-scan run."""
...
from experiments.consciousness_sae_signed_dose_scan import (  # noqa: E402
    authorize,
    orientation,
    protocol,
    validate_plan,
...
EXPECTED_PAIR_COUNT = len(protocol.rows())
...
PAIRS_PER_PROMPT = len(protocol.DIRECTIONS) * len(protocol.DOSE_GRID)
BRANCHES_PER_PROMPT = 2 * PAIRS_PER_PROMPT
CLEAN_STATE_COUNT = len(protocol.J_LAYERS) + 1
TRACE_STATE_COUNT = len(protocol.J_LAYERS) + 2
...
def _fixed_token_panel() -> tuple[int, ...]:
    ...
    offset = protocol.seed64(
        protocol.FRESH_RANDOMIZATION_SPEC["fixed_token_panel_seed_namespace"]
    ) % modulus
...
def _audit_runtime_and_binding(
    ...
    *,
    ...
    expected_seed = protocol.seed64(
        protocol.FRESH_RANDOMIZATION_SPEC["runtime_seed_namespace"]
    ) % (2**63 - 1)
    if (
...
def _random_j_parameters(
    ...
    rng = np.random.Generator(
        np.random.PCG64(
            protocol.seed64(
                protocol.FRESH_RANDOMIZATION_SPEC["random_j_seed_namespace"],
                layer,
                index,
            )
        )
...
def audit(
    ...
    *,
    ...
    if tuple(root.parts[-4:]) != expected_tail:
        raise CalibrationAuditError("raw run is outside the exact signed-dose namespace")
    ...
    if (
        len(realization_rows) != EXPECTED_PAIR_COUNT
        or len(pair_rows) != EXPECTED_PAIR_COUNT
        or len(readout_rows) != EXPECTED_READOUT_COUNT
        or len(prompt_rows) != len(protocol.PROMPT_IDS)
        or len(clean_index) != len(protocol.PROMPT_IDS)
    ...
    if tuple(clean.shape) != (
        len(protocol.PROMPT_IDS),
        CLEAN_STATE_COUNT,
        protocol.WIDTH,
    ) or clean.dtype != torch.bfloat16:
    ...
    for prompt_offset, prompt_id in enumerate(protocol.PROMPT_IDS):
        ...
        if (
            tuple(arcs.shape)
            != (BRANCHES_PER_PROMPT, TRACE_STATE_COUNT, protocol.WIDTH)
            or arcs.dtype != torch.bfloat16
        ):
            raise CalibrationAuditError(f"signed residual shard differs: {prompt_id}")
        expected_arithmetic = {
            "requested_fp32": (torch.float32, (PAIRS_PER_PROMPT, protocol.WIDTH)),
            "requested_bfloat16": (
                torch.bfloat16,
                (PAIRS_PER_PROMPT, protocol.WIDTH),
            ),
            "realized_plus_fp32": (
                torch.float32,
                (PAIRS_PER_PROMPT, protocol.WIDTH),
        ...
        expected_readout = {
            "source_delta_fp32": (
                torch.float32,
                (len(protocol.DIRECTIONS), len(protocol.READOUT_LAYERS), protocol.WIDTH),
            ),
        ...
        for pair_offset in range(PAIRS_PER_PROMPT):
            ...
            clean_final = clean[prompt_offset, -1].float()
            final_plus = plus[-1].float() - clean_final
            final_minus = minus[-1].float() - clean_final
            final_common = (
                plus[-1].float() + minus[-1].float()
            ) * 0.5 - clean_final
            ...
            recomputed_realization.append(
                {
                    ...
                    "dose_fraction": dose,
                    **metrics,
                    "final_plus_rms_fraction_of_clean_final": (
                        _rms(final_plus) / _rms(clean_final)
                    ),
                    "final_minus_rms_fraction_of_clean_final": (
                        _rms(final_minus) / _rms(clean_final)
                    ),
                    "final_central_rms_fraction_of_clean_final": (
                        _rms(final) / _rms(clean_final)
                    ),
                    "final_common_mode_to_central_rms": _safe_ratio(
                        _rms(final_common), _rms(final)
                    ),
                    "downstream_rms_gain_over_realized_source": _safe_ratio(
                        _rms(final), _rms(realized)
                    ),
            ...
            if dose == protocol.PRIMARY_DOSE:
                ...
                for layer_offset, layer in enumerate(protocol.READOUT_LAYERS):
                    ...
                    for transport_offset, transport_name in enumerate(
                        ...
                        readout_cursor += 1
            raw_cursor += 1
    if raw_cursor != EXPECTED_PAIR_COUNT or readout_cursor != EXPECTED_READOUT_COUNT:
    ...
    dose_summaries: dict[str, Any] = {
        "0.0": {
            "dose_basis_points": 0,
            "role": "single_shared_clean_baseline",
            "row_count": len(protocol.PROMPT_IDS),
            "direction_specific_duplicates": False,
            "actual_final_state_curve": {
                "final_plus_rms_fraction_of_clean_final": 0.0,
                "final_minus_rms_fraction_of_clean_final": 0.0,
                "final_central_rms_fraction_of_clean_final": 0.0,
                "final_common_mode_to_central_rms": 0.0,
                "downstream_rms_gain_over_realized_source": None,
            },
        }
    }
        ...
        dose_summaries[str(dose)] = {
            ...
            "actual_final_state_curve": {
                ...
                for field in (
                    ...
                    "final_minus_rms_fraction_of_clean_final",
                    "final_central_rms_fraction_of_clean_final",
                    "final_common_mode_to_central_rms",
    ...
    summary_core = {
        ...
        **claim_statuses,
        ...
        "diagnostic_non_gate_dose_failure_count": len(diagnostic_failures),
        "j_shadow_gate_failure_count": len(j_shadow_failures),
        "diagnostic_non_gate_dose_j_shadow_failure_count": len(
            diagnostic_j_shadow_failures
        ),
