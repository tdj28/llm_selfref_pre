...
def _validate_c3_qualification_for_c4(
    qualification_dir: Path,
) -> dict[str, Any]:
    """Authenticate the successful, outcome-blind E3 qualification receipts."""

    root = qualification_dir.expanduser().absolute()
    if root.name != C3_QUALIFICATION_DIRECTORY_NAME:
        raise QualificationIncidentError("C3 qualification namespace differs")
    try:
    ...
    if names != sorted(C3_QUALIFICATION_EVIDENCE_SPECS):
        raise QualificationIncidentError("C3 qualification inventory differs")
    ...
    if (
        ...
        or qualification.get("successor_authority_binding_sha256")
        != C3_AUTHORITY_BINDING_SHA256
        ...
        or fresh_pod.get("model_forward_count") != 0
        or fresh_pod.get("target_prompt_render_count") != 0
        ...
        raise QualificationIncidentError("C3 qualification semantics differ")
    return {
...
def successor_c4_authority_binding(
    ...
    """Bind E3's successful qualification to explicit one-shot C4 authority."""

    predecessor = _validate_c3_qualification_for_c4(
    ...
    if (
        ...
        raise QualificationIncidentError("C4 successor authority differs")
    return {
        **expected_authority,
...
def main() -> int:
    ...
    if args.recovery_cycle_ledger is None:
        value = validate_incident(args.incident_dir)
