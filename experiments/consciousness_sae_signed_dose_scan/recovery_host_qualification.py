...
# Kept as a source-compatible alias for the already frozen controller CLI.  In
# C4 this argument names the predecessor qualification evidence directory, not
# the older C2 incident directory.
...
def _validated_c4_successor_authority(value: Any) -> dict[str, Any]:
    """Restate the complete C4 authority contract at the execution boundary."""
    ...
    if not isinstance(value, Mapping) or dict(value) != expected:
        raise RecoveryHostQualificationError("C4 successor authority differs")
    return dict(value)
...
def qualify_host(
    *,
    ...
    if enforce_git and QUALIFICATION_AUTHORITY_STATE != "authorized":
        raise RecoveryHostQualificationError(
            "C4 qualification lacks explicit post-C3 human authority"
        )
