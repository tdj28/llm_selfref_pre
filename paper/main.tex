...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  ...
  \item a clean-room reanalysis of a public AE steering notebook, balanced and dual-paraphraser 70B public-SAE activation maps with lexical counterfactuals, adaptive target-versus-active-random tests, and a prospectively frozen 1,500-trial full-grid replication with three matched control panels; and
...
The evidence supports an interpretation-level challenge, not an accusation about author intent and not a demonstration that models lack consciousness. The most defensible statement is: the reported dependent variables are underidentified. Active instruction and query packages cause large changes in the tested designs; register point estimates, response-model differences, and evaluator differences show additional sensitivity. The public SAE features encode broad narrative/social language whose aggregate ordering survives two paraphraser families but is materially recreated by lexical cue transplant. Their prospectively tested public-weight aggregate does not produce the reported paper-direction effect at either the literal or calibrated scale, and three matched controls provide no positive specificity result. This is a non-replication of a disclosed public implementation, not a falsification of inaccessible proprietary software. More specific and independently accessible evidence is required before connecting these observations to subjective experience.
...
Self-referential prompting can reproducibly cause some language models to emit text that paper-style judges classify as subjective-experience reports. Our strongest behavioral result is that this effect follows the active written instruction when the generated transcript is transplanted, while orthogonal self-reference effects are weak and query/model interactions are large. That endpoint is conditional on an automated rubric; it is not ground truth for subjective experience attribution. The benchmark therefore identifies instruction-conditioned language but does not identify a self-reference-specific phenomenal state. Public SAE mapping preserves a useful mechanistic lead, but the candidate features encode broad fiction, roleplay, concealment, and dishonesty semantics, and their activation ordering crosses a prospectively frozen lexical-entanglement threshold. Most importantly, the prospectively frozen 1,500-trial public-weight intervention yields no paper-direction target effect at the literal scale, no rescue at the larger calibrated scale, no corrected individual-feature slope, and no positive specificity result across three matched panels. This is strong evidence against the signature under our disclosed public implementation, while the inaccessible proprietary API remains outside the claim. The appropriate next step is narrower measurement, independent human coding, natural-corpus feature validation, and an accessible proprietary or open mechanistic replication, not a larger metaphysical conclusion.
...
\begin{table}[H]
\centering
\caption{Primary causal and prospective public-SAE release inventory.}
\small
\begin{tabular}{lr}
\toprule
Artifact & Rows \\
...
Every JSONL identifier is unique within its artifact. The release manifests record file hashes, four empty causal-study outcomes, the retained transient causal judge-format error, and two zero-row public-SAE judge startup failures. Private keys, Hugging Face tokens, coder linkage keys, and model caches are excluded.
