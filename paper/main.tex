...
\paragraph{Jacobian-lens steering.}
The Jacobian lens derives layer-specific, token-indexed residual directions from the corpus-averaged first-order map between intermediate activations and current or future outputs \citep{gurnee2026workspace}. Unlike an SAE dictionary trained to reconstruct the broad activation distribution, its directions are constructed to identify verbalizable content. The associated J-space ablation dynamically removes strongly active verbalizable directions across a layer band. It reduces experiential language in both model self-reports and descriptions of other people, which makes it a complementary causal result about experiential reporting register rather than a self-specific consciousness measure. This intervention is not equivalent to editing a fixed set of deception-labeled SAE coordinates.
...
Goodfire's legacy Llama SAE pages now state that its SAE demo interface and API were deprecated in February 2026 \citep{goodfire2024mapping}; the former documentation URL redirects to Goodfire's current site. A separately branded service at \url{https://www.steeringapi.com/} remained publicly reachable when checked on 2026-07-11, but no public artifact establishes that it is the legacy Goodfire backend or that its current model, SAE, feature namespace, and intervention semantics match the paper-time configuration. Exact replication therefore requires archival access or a frozen paper-time manifest; a current SteeringAPI execution would be a separate current-service result unless equivalence is demonstrated.
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
  ...
  \item The target paper's proprietary SAE/API configuration is unavailable, and Goodfire's legacy SAE API is now deprecated. The separately reachable SteeringAPI is not publicly established as equivalent. The accepted notebook IDs and public layer-50 weights do not establish equivalence of feature lookup, quantization, hook semantics, or intervention scale. The full-grid verdict therefore names the public implementation explicitly.
